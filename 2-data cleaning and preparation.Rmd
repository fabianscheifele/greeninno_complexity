...
6. Replace deleted CPC codes by latest revisions
```{r}
cpc_changes<-read_xlsx(here("data","raw","cpc_changes.xlsx"))

#remove white space
cpc_changes$old_cpc<-str_replace(cpc_changes$old_cpc," ","")
cpc_changes$new_cpc<-str_replace(cpc_changes$new_cpc," ","")
y2_final95_distinct$cpc<-str_replace(y2_final95_distinct$cpc," ","")

#merge with data
y2_final95_distinct<-y2_final95_distinct%>%
  left_join(cpc_changes, by=c("cpc"="old_cpc"))

sum(is.na(y2_final95_distinct$new_cpc))/length(y2_final95_distinct$appln_id)
#less than 0.05% of observations are affected by this

#replace codes that have been replaced by other codes
y2_final95_distinct<-y2_final95_distinct%>%
 mutate(cpc_updated=case_when(is.na(new_cpc)~cpc,TRUE~new_cpc))

sum(is.na(y2_final95_distinct$cpc_updated))
#replacement worked, not NA's anymore#

#Replace the remaining 13 codes that were filed but no longer exist manually
y2_final95_distinct$cpc_updated[y2_final95_distinct$cpc_updated=="Y02D10/13"
 | y2_final95_distinct$cpc_updated=="Y02D10/13"
| y2_final95_distinct$cpc_updated=="Y02D10/126"
| y2_final95_distinct$cpc_updated=="Y02D10/171"
| y2_final95_distinct$cpc_updated=="Y02D10/172"
| y2_final95_distinct$cpc_updated=="Y02D10/173"
| y2_final95_distinct$cpc_updated=="Y02D10/122"
| y2_final95_distinct$cpc_updated=="Y02D10/16"
| y2_final95_distinct$cpc_updated=="Y02D10/14"
| y2_final95_distinct$cpc_updated=="Y02D10/151"
| y2_final95_distinct$cpc_updated=="Y02D10/153"]<-"Y02D10/00"

y2_final95_distinct$cpc_updated[y2_final95_distinct$cpc_updated=="Y02D70/00"]<-"Y02D30/70"

#B60/126 was actually moved to Y02D/157 but then this was again replaced by Y02D10/00
y2_final95_distinct$cpc_updated[y2_final95_distinct$cpc_updated=="Y02B60/126"]<-"Y02D10/00"

y2_final95_distinct$cpc_updated[y2_final95_distinct$cpc_updated=="Y02T50/14"]<-"Y02T50/10"

#Check with latest description file to see any missings (i.e. subgroups that do not exist)
cpc_desc<-read_xlsx(here("data","raw","cpc_description.xlsx"))
cpc_desc$CPC<-str_replace(cpc_desc$CPC," ","")
y2_final95_distinct<-y2_final95_distinct%>%
  left_join(cpc_desc, by=c("cpc_updated"="CPC"))

cpc_notfound<-y2_final95_distinct%>%filter(is.na(description))%>%distinct(cpc_updated)


#dummy to mark them
y2_final95_distinct<-y2_final95_distinct%>%
 mutate(cpc_replaced=case_when(cpc==cpc_updated~0,TRUE~1))

#remove text columns
y2_final95_distinct<-y2_final95_distinct%>%
  select(-c("description","level"))

#removing any final duplicates of CPCs that may have been created through CPC renaming
y2_final95_distinct<-y2_final95_distinct%>%
  distinct(appln_id, person_id, cpc, .keep_all=TRUE)
```
...
7. Checking out missing country information for IPF's
```{r}
missing_country<-int_pat_fam_final%>%
  filter(is.na(person_ctry_code))

length(unique(missing_country$docdb_family_id))/length(unique(int_pat_fam_final$docdb_family_id))
#For 58.4% of all IPF inventor residence info is incomplete. 

family_found<-int_pat_fam_final%>%filter(!is.na(person_ctry_code))%>%
  distinct(docdb_family_id)

length(family_found$docdb_family_id)/length(unique(int_pat_fam_final$docdb_family_id))
#For 98.4% of International Patent Families we have at least one inventor's residence info

missing_country%>%group_by(appln_auth)%>%
  summarise(top_missing=n_distinct(docdb_family_id))%>%
  slice_max(top_missing, n=10)
#Patent offices with most missing info are CN, JP, AU, KR, ES, MX
```

8. Attempt to recuperate remaining missing data through matching with other applications
that were dropped by distinct: Nothing found! 
```{r}

#match with original raw data
#country_exist<-y2_final95_distinct%>%filter(!is.na(person_ctry_code))
...
#countries_found<-left_join(missing_country,countries_exist_dis, by=c("person_id"))
#Does not find any missing person information
#sum(is.na(countries_found$person_ctry_code.y))
```

9. Dropping remaining NA's for person_ctry_code and creating final data set
```{r}
...
int_pat_fam_final%>%group_by(appln_auth)%>%
  summarise(count=n_distinct(docdb_family_id))%>%arrange(desc(count))

int_pat_fam_final_complete%>%
  group_by(appln_auth)%>%
  summarise(count=n_distinct(docdb_family_id))%>%arrange(desc(count))
#Japan has almost zero origin data, China families also reduced by 90%

int_pat_fam_final%>%filter(is.na(person_ctry_code))%>%
  group_by(appln_auth)%>%
  summarise(count=n_distinct(docdb_family_id))%>%arrange(desc(count))
...
#IF WANTED FOR ROBUSTNESS ONE COULD replace npw the missing country values in int_pat_fam_final with the application authority of the priority patent! 
```
