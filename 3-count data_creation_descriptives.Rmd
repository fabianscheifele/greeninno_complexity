...
balassa_matrix_by_year <- function(t) {
  d_year<- pat_count_country_reduced %>% filter(year == t)
...
summary(d_year)
d_year<- pat_count_country_reduced %>%
  filter(year == 1995)%>%
  select(1:4)
d_year<-as.data.frame(d_year)
d_year$country<-as.factor(d_year$country)
balassa_mat<-balassa_index(data = d_year, country = "country", 
                           product = "cpc_level12", 
                           value = "ipf_count", 
                           cutoff = 1,
                           discrete = TRUE)

y_1995<-balassa_matrix_by_year(1995)
```
